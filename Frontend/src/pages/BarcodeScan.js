...
const BarcodeScan = () => {
  ...
  const handleSuccessfulScan = (qrValue) => {
    ...
    setTimeout(() => {
      console.log(
        `Successfully scanned QR code: ${qrValue} for delivery ${deliveryId}`
      );
      setSuccess(true);
      setLoading(false);
      ...
      setTimeout(() => {
        navigate(`/delivery-complete/${deliveryId}`);
      }, 1500);
    }, 1000);
  };
