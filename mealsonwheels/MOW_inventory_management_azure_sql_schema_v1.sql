...
CREATE TABLE Client (
    ...
    service_route_id UNIQUEIDENTIFIER NOT NULL,
    status NVARCHAR(20) NOT NULL DEFAULT 'active',
    notes NVARCHAR(MAX),
    created_at DATETIME2 NOT NULL DEFAULT SYSDATETIME(),
    updated_at DATETIME2 NOT NULL DEFAULT SYSDATETIME(),
    CONSTRAINT FK_Client_ServiceRoute FOREIGN KEY (service_route_id) 
...
CREATE TABLE DeliveryItem (
    ...
    CONSTRAINT FK_DeliveryItem_Client FOREIGN KEY (client_id) 
...
-- Create needed indexes for performance optimization in Azure SQL Database
...
CREATE INDEX IX_Client_ServiceRouteId ON Client(service_route_id);
CREATE INDEX IX_Client_Status ON Client(status);
CREATE INDEX IX_Client_FullName ON Client(last_name, first_name);
CREATE INDEX IX_Client_ZipCode ON Client(zip_code);
...
CREATE INDEX IX_DeliveryItem_ClientId ON DeliveryItem(client_id);
...
CREATE INDEX IX_Client_RouteStatus ON Client(service_route_id, status);
CREATE INDEX IX_DeliveryItem_DeliveryClient ON DeliveryItem(delivery_id, client_id);
...
SELECT 
    ...
    COUNT(DISTINCT di.client_id) AS total_clients
FROM 
    Delivery d
...
-- View for client meal history
CREATE OR ALTER VIEW vw_ClientMealHistory AS
SELECT 
    c.client_id,
...
FROM 
    Client c
    JOIN DeliveryItem di ON c.client_id = di.client_id
...
ORDER BY 
    c.client_id, d.delivery_date DESC;
GO
